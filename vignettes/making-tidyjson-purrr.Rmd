...
```{r}
structure <- samp_co[[1]] %>% json_structure

max_length <- 20
structure %>%
  mutate(json = structure %@% "JSON") %>%
  transmute(child.id, key, type, length, 
    json = ifelse(!(type %in% c("object", "array")), as.character(json), NA_character_))%>%
  mutate(json = paste0(json %>% str_sub(end = max_length - 3),
                       ifelse(str_length(json) > max_length - 3, "...", "")))
```

Hard to understand this using ggplot

```{r}
structure %>%
  replace_na(list(key = "NA")) %>%
