...
rounds <- companies %>%
  ...
  gather_array %>%
  spread_values(
    round = json_chr(round_code),
    currency = json_chr(raised_currency_code),
    raised = json_dbl(raised_amount)
  )
rounds %>% head
```
...
geos <- companies %>%
  ...
  gather_array %>%
  spread_values(
    country = json_chr(country_code),
    state = json_chr(state_code),
    description = json_chr(description)
  )
geos %>% head
```
