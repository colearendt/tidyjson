...
Suppose we want to find out how much each person has spent. We can begin by inspecting the type of the JSON using `json_types`:

```{r}
purch_json %>% json_types
```

The top level object is an array (it begins with `[`), so we use `gather_array`
to gather each array element into it's own row:

```{r}
purch_json %>% gather_array
```

Now each JSON begins with a `{`, indicating it is an object, so we use 
`spread_all` to capture top level keys (in this case, `"name"`):

```{r}
purch_json %>% gather_array("customer.num") %>% spread_all
```

Now we want to access the purchase data, so let's enter into the `"purchases"`
object  using `enter_object`
...
As you can see from the JSON attribute, the `"purchases"` object is an array,
and so we call `gather_array` and `spread_all` again:

```{r}
purch_json %>% gather_array("customer.num") %>% spread_all %>%
  enter_object("purchases") %>% gather_array("purchase.num") %>% spread_all
```

Now we can enter into the final array, `"items"` with a similar pipeline:

```{r}
purch_json %>% gather_array("customer.num") %>% spread_all %>%
  enter_object("purchases") %>% gather_array("purchase.num") %>% spread_all %>%
  enter_object("items") %>% gather_array("item.num") %>% spread_all
```
...
y <- c('{"name1": "value1"}', '{"name2": "value2"}') %>% as.tbl_json
y
```
...
df <- data.frame(
  x = 1:2,
  JSON = c('{"name1": "value1"}', '{"name2": "value2"}'),
